...
<main>
    <?php
        $q = $mysqli -> query("SELECT * FROM posts WHERE category = 'breakfast'");
        while($r = $q -> fetch_assoc()){
            echo '
                <div class="posts"><h1>'.$r["title"].'</h1>
                '.$r["content"].'
                <a href="deletePost.php?id='.$r['id'].'&return=breakfast">Delete</a>
                </div>
            ';
        }
    ?>
    <br>
    <a href="addPost.php?category=breakfast">Add new recipe</a>
</main>
<footer>
